...
internal fun List<PlayerRosterEntry>.availableNicknameClosestTo(preferredNickname: Nickname): Nickname =
    availableNicknameClosestTo(preferredNickname, null)

private fun List<PlayerRosterEntry>.availableNicknameClosestTo(preferredNickname: Nickname, index: Int?): Nickname =
    preferredNickname.suffixedWith(index).let { nickname ->
        if (none { it.nickname == nickname }) nickname
